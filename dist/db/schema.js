import { integer, pgTable, text, timestamp, check, numeric, boolean, index, pgEnum, customType, uniqueIndex, } from 'drizzle-orm/pg-core';
...
export const whatsappConnectionStatusEnum = pgEnum('whatsapp_connection_status', ['pending', 'qr_ready', 'authenticated', 'ready', 'disconnected', 'failed']);
...
export const vagas = pgTable('vagas', {
    id: integer('id').primaryKey().generatedAlwaysAsIdentity(),
    connectionId: integer('connection_id').references(() => whatsapp_connections.id),
    title: text('title'),
...
export const whatsapp_connection_groups = pgTable('whatsapp_connection_groups', {
    id: integer('id').primaryKey().generatedAlwaysAsIdentity(),
    ...
    uniqueIndex('whatsapp_connection_groups_connection_group_unique').on(table.connectionId, table.groupId),
]);
