...
class WhatsappConnectionManager {
    ...
    async updateConnectionWithRetry(connectionId, values, context, maxAttempts = 3) {
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                this.logRuntimeError(`${context} attempt=${attempt}/${maxAttempts}`, connectionId, error);
    ...
    attachLifecycleHandlers(connectionId, clientKey, client) {
        client.on('qr', async (qr) => {
            try {
                await this.updateConnectionWithRetry(connectionId, {
                    status: 'qr_ready',
                    lastQr: qr,
                    lastQrAt: new Date(),
                }, 'event:qr');
        ...
        if (!runtime.initialized) {
            ...
            void Promise.resolve(initializeResult).catch((error) => {
                void (async () => {
                    ...
                    this.runtimes.delete(connectionId);
                    await this.updateConnectionWithRetry(connectionId, { status: 'failed', disconnectedAt: new Date() }, 'initialize:failed');
                    emitWhatsappRuntimeEvent(connectionId, {
                        type: 'error',
                        payload: {
                            status: 'failed',
