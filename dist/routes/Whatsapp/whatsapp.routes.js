...
export const whatsappRoutes = async (app) => {
    app.get('/whatsapp/connections', {
        ...
        const connections = await withDbRetry(() => db
            .select()
            .from(whatsapp_connections)
    ...
    app.delete('/whatsapp/connections/:id/delete', {
        ...
        await withDbRetry(() => db
            ...
            .execute());
        const deletedConnection = await withDbRetry(() => db
    ...
    app.get('/whatsapp/connections/:id/groups', {
        ...
        for (const chat of chats) {
            ...
            if (!whatsappId || !isSelectableGroup)
                continue;
            let group = await withDbRetry(() => db
                .select()
                .from(grupos_whatsapp)
                .where(eq(grupos_whatsapp.whatsaapId, whatsappId))
                .then((rows) => rows[0]));
            if (!group) {
                const [insertedGroup] = await withDbRetry(() => db
            ...
            await withDbRetry(() => db
                ...
                .onConflictDoNothing()
                .execute());
            const relation = await withDbRetry(() => db
                .select()
