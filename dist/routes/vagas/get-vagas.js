...
export const getVagasFilters = async (app) => {
    app.get('/vagas/filtros', {
        ...
        const filters = [];
        if (category)
            filters.push(eq(vagas.category, category));
        if (modality)
            filters.push(eq(vagas.modality, modality));
        if (tipo_vaga)
            filters.push(eq(vagas.tipo_vaga, tipo_vaga));
        if (location)
            filters.push(eq(vagas.location, location));
        if (publisheAt)
            filters.push(eq(vagas.publisheAt, new Date(publisheAt)));
        const resultFilter = await withDbRetry(() => db
            .select()
            .from(vagas)
            .where(and(...filters))
            .orderBy(desc(vagas.publisheAt))
