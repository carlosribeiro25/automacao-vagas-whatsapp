...
test('Retorna 409 ao listar grupos sem runtime iniciado', async () => {
    await server.ready()
...
test('Seleciona grupos monitorados da conexao', async () => {
    ...
    const [groupA] = await db.insert(grupos_whatsapp).values({
        name: 'Grupo A',
        whatsaapId: faker.string.uuid(),
    }).returning()

    const [groupB] = await db.insert(grupos_whatsapp).values({
        name: 'Grupo B',
        whatsaapId: faker.string.uuid(),
    }).returning()
